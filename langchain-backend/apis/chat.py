...
@router.post("/chat_agent", response_model=QueryResponse)
def chat(query_input: QueryInput):
    session_id = query_input.session_id or str(uuid.uuid4())
    logging.info(f"Session ID: {session_id}, User Query: {query_input.question}, Model: {query_input.model.value}")
...
@router.post("/chat_graph", response_model=QueryResponse)
def chat(query_input: QueryInput):
    session_id = query_input.session_id or str(uuid.uuid4())
    logging.info(f"Session ID: {session_id}, User Query: {query_input.question}, Model: {query_input.model.value}")
    ...
    answer_graph = graph.invoke(
        input={
            "messages": [{"role": "user", "content": query_input.question}],
        },
        config={
            "configurable": {
    ...
    answer = answer_graph['messages'][-1].content
